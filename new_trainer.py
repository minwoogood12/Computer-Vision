...
def pad_image_list(image_list, size_divisibility=32):
      ...
      for img in image_list:
        ...
        pad_bottom = max_h - h
        pad_right = max_w - w
        padded = F.pad(img, (0, pad_right, 0, pad_bottom))  # (left, right, top, bottom)
        padded_images.append(padded)

      return torch.stack(padded_images)  # [B*T, C, H_max, W_max]
...
class Trainer:
    ...
    def _step(
        ...
        B, T = images.shape[:2]
        image_list = [images[b, t] for b in range(B) for t in range(T)] 
        #image : List[Tensor(C,H,W) * B*T]
        ...
        images_lab_sim_nei7 = [get_neighbor_images_patch_color_similarity(images_lab[ii+7].unsqueeze(0), images_lab[ii].unsqueeze(0), 3, 3) for ii in range(0, len(images_lab), 8)]
