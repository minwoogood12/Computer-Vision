...
class VideoSetCriterion(nn.Module):
    ...
    def loss_masks_proj(self, outputs, targets, indices, num_masks, images_lab_sim, images_lab_sim_nei, images_lab_sim_nei1, images_lab_sim_nei2, images_lab_sim_nei3, images_lab_sim_nei4):
        ...
        src_mask = src_masks.detach().cpu()
        N_obj, N_mask, H, W = src_mask.shape

        for i in range(N_obj):      # 객체 loop
            for j in range(N_mask): # 후보 loop
                mask = src_mask[i, j]
                binary_mask = (mask.sigmoid() > 0.5).float() * 255.0
                binary_mask = binary_mask.numpy().astype("uint8")
                img = Image.fromarray(binary_mask)
                save_path = f"./images/mask_obj{i}_cand{j}.png"
                img.save(save_path)
                print(f"Saved: {save_path}")
        # Modified to handle video
        target_masks = torch.cat([t['masks'][i] for t, (_, i) in zip(targets, indices)]).to(src_masks)

        #src_masks : 3 5 96 160
        #target_masks : 3 5 384 640

        #image_lab_sim : 10 ( 8 96 160 )
        #image_lab_sim_nei : 2 ( 9 96 160 )
        ...
        #image_lab_sim : 10 8 96 160 
        #image_lab_sim_nei : 2 9 96 160 => 이거 원래 리스트형태를 텐서로푼거
        ...
        #image_lab_sim : 2 5 8 96 160 
        #image_lab_sim_nei : 2 1 9 96 160
        #이거 이제 그 배치와 프레임 구분 

        if len(src_idx[0].tolist()) > 0:
            images_lab_sim = torch.cat([images_lab_sim[ind][None] for ind in src_idx[0].tolist()]).flatten(0, 1)

            images_lab_sim_nei = torch.cat([images_lab_sim_nei[ind][None] for ind in src_idx[0].tolist()]).flatten(0, 1)
            images_lab_sim_nei = self.topk_mask(images_lab_sim_nei, 5)

            #images_lab_sim_nei = self.topk_mask(torch.cat([images_lab_sim_nei[ind][None] for ind in src_idx[0].tolist()]).flatten(0, 1), 5)            images_lab_sim_nei1 = self.topk_mask(torch.cat([images_lab_sim_nei1[ind][None] for ind in src_idx[0].tolist()]).flatten(0, 1), 5)
        ...
        #여기서는 이제 영상 2개고 0번영상에 객체 1개매치, 1번영상에 객체 2개치
        #즉 images_lab_sim은 2배치에서 1번은 1개 2번은 2개복사해서 3개로이어붙임 
        #3 5 8 96 160만들고 이후 맨 앞두개 연결 -> 15 8 96 160
        #images_lab_sim_nei는 이제 2배치에서 똑같이 1번은 1개 2번은 2개 복사해서 3개로붙이고
        #3 1 9 96 160 만들고 맨앞 두개 연결 -> 3 9 96 160 -> 이후 주변 9개중에 5개만 남기기 작업

        #즉 그 사무라이에서는 그냥 객체 수만큼 맨앞에 해주면 될듯
