...
class Trainer:
    ...
    def _step(
        ...
        #maskfreevis List[Tensor(3,H,W) * B * T 배치사이즈] 
        #SAM2 image_batch : [T,B,C,H,W] 즉 이거를 이미지텐서 * B, T로 바꿔야함
        #targets : [T, O, H, W]
      
        T, B, C, H, W = batch.img_batch.shape ##디버깅 수정
        images = list(batch.img_batch.permute(1, 0, 2, 3, 4).reshape(-1, C, H, W))
        #images : List[Tensor(3,H,W) * (B*T)]
      
        k_size = 3 #3
        rs_images = torch.stack(images)
        #rs_images : Tensor(B*T,3,H,W)
      
        downsampled_images = F.avg_pool2d(rs_images.float(), kernel_size=4, stride=4, padding=0) #for img in images]
        ...
        images_lab_sim_nei4 = [get_neighbor_images_patch_color_similarity(images_lab[ii+4].unsqueeze(0), images_lab[ii+5].unsqueeze(0), 3, 3) for ii in range(0, len(images_lab), 8)]##추가##
        images_lab_sim_nei5 = [get_neighbor_images_patch_color_similarity(images_lab[ii+5].unsqueeze(0), images_lab[ii+6].unsqueeze(0), 3, 3) for ii in range(0, len(images_lab), 8)]
        images_lab_sim_nei6 = [get_neighbor_images_patch_color_similarity(images_lab[ii+6].unsqueeze(0), images_lab[ii+7].unsqueeze(0), 3, 3) for ii in range(0, len(images_lab), 8)]
        images_lab_sim_nei7 = [get_neighbor_images_patch_color_similarity(images_lab[ii+7].unsqueeze(0), images_lab[ii].unsqueeze(0), 3, 3) for ii in range(0, len(images_lab), 8)]##추가##
