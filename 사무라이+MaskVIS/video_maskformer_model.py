...
class VideoMaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            #(3,H_pad, W_pad)의 색상 순서 [B,G,R] -> [R,G,B]바꾸고 차원순서 바꿔서
            #List[Tensor(H_pad, W_pad, 3)] 길이 : B * T
            images_lab_sim = [get_images_color_similarity(img_lab.unsqueeze(0), k_size, 2) for img_lab in images_lab] # ori is 0.3, 0.5, 0.7
            #unsqueeze로 [1,3,H,W] 후 자신과 주변의 픽셀의 유사도를 나타내는 이미지 유사도 (0~1)
            #List[Tensor(H_pad, W_pad, 3)] 길이 : B * T
            ...
            #unsqueeze로 [1,3,H,W] 후 이웃과의 유사도를 나타내는 이미지 유사도 (0~1)
            #List[Tensor(H_pad, W_pad, 3)] 길이 : B
