...
class MultiStepMultiMasksAndIous(nn.Module):
    ...
    def forward(self, outs_batch: List[Dict], targets_batch: torch.Tensor,
        ...
        loss_tk, loss_pairwise, loss_proj = self.loss_masks_proj(mfvis_masks, mfvis_gt,
                                                       images_lab_sim,
                                                       images_lab_sim_nei,
                                                       images_lab_sim_nei1,
                                                       images_lab_sim_nei2,
    ...
    def convert_predict(self, outs_batch: List[Dict]) -> torch.Tensor:
      pred_masks_per_frame = []
      for frame_out in outs_batch:
    ...
    def convert_gt(self, gt_masks: torch.Tensor) -> torch.Tensor:
      assert gt_masks.ndim == 4, "Expected 4D tensor [T, N, H, W]"
      T, N, H, W = gt_masks.shape

      # Permute to [N, T, H, W]
      converted = gt_masks.permute(1, 0, 2, 3).contiguous()
    ...
    def loss_masks_proj(self, outputs, targets,
        ...
        for obj_idx in range(N):
            for frame_idx in range(T):
                # --- Output mask 저장 ---
                out_mask = outputs_np[obj_idx, frame_idx]  # (H, W)
                out_mask = (out_mask > 0.5).float()  # 이진화
                out_img = TF.to_pil_image(out_mask)
                out_img.save(os.path.join(output_dir, f"obj{obj_idx}_frame{frame_idx}.png"))

                # --- Target mask 저장 ---
                tgt_mask = targets_np[obj_idx, frame_idx]
                tgt_mask = (tgt_mask > 0.5).float()
                tgt_img = TF.to_pil_image(tgt_mask)
                tgt_img.save(os.path.join(target_dir, f"obj{obj_idx}_frame{frame_idx}.png"))
        ...
        #print("🔹 src_masks shape:", src_masks.shape)       # Expected: [N, T, H, W]
        #print("🔹 target_masks shape:", target_masks.shape) # Expected: [N, T, H, W]

        

        # 명시적으로 [N, T, H, W] → [N*T, 1, H, W]
        N, T, H, W = src_masks.shape
        src_masks_4d = src_masks.reshape(N * T, 1, H, W)
        target_masks_4d = target_masks.view(N * T, 1, H, W)
        ...
        target_masks_down = F.interpolate(
          target_masks_4d.float(),
            scale_factor=0.25,
            mode="bilinear",
            align_corners=False
        )
        ...
        target_masks = target_masks_down.view(N, T, H_, W_)
        #print("🔹 src_masks shape:", src_masks.shape)       # Expected: [N, T, H, W]
        #print("🔹 target_masks shape:", target_masks.shape)

        images_lab_sim = torch.cat(images_lab_sim, dim =0)
        #List[Tensor(1, K^2,H,W) * (B*T)]를 1차원에 길이만큼 이어붙임
        # => Tensor(B*T,K^2,H,W)
        #print("🟢 images_lab_sim shape after cat:", images_lab_sim.shape)

        images_lab_sim_nei = torch.cat(images_lab_sim_nei, dim=0)
        #print("🟢 images_lab_sim_nei shape after cat:", images_lab_sim_nei.shape)
        ...
        #Tensor(B,T,K^2,H,W)
        #print("🔵 images_lab_sim shape after view:", images_lab_sim.shape)

        images_lab_sim_nei = images_lab_sim_nei.unsqueeze(1)
        #Tensor(B,1,K^2,H,W)
        #print("🔵 images_lab_sim_nei shape after unsqueeze:", images_lab_sim_nei.shape)
        ...
        #print("🟣 images_lab_sim shape after flatten(0,1):", images_lab_sim.shape)
        ...
        #[N,K^2,H,W]
        #print("🟣 images_lab_sim_nei shape after topk_mask:", images_lab_sim_nei.shape)
        ...
        if src_masks.shape[0] > 0:
            ...
            #print("🟠 pairwise_losses_neighbor shape:", pairwise_losses_neighbor.shape)

            #예측마스크[N, T, H, W] 의 객체 별로 각가 0번프레임[N,1,H,W], 1번프레임[N,1,H,W] 비교
            #[N, K^2, H, W] 예측된 마스크 n개에 대해 0번 1번 프레임이 얼마나 비슷한 예측인지 비교한 map (0~0.69) 0이면 일치, 0.69면 일치안함
        ...
        #print("🔴 src_masks shape after flatten and unsqueeze:", src_masks.shape)

        target_masks = target_masks.flatten(0, 1)[:, None]
        #0,1차원을 합치고 1차원 추가 생성 즉 [N*T, 1, H, W]
        #print("🔴 target_masks shape after flatten and unsqueeze:", target_masks.shape)

        target_masks = F.interpolate(target_masks, (src_masks.shape[-2], src_masks.shape[-1]), mode='bilinear')
        #정답마스크해상도를 예측에 맞게 리사이즈
        #print("🔴 target_masks shape after interpolate:", target_masks.shape)
        ...
        if src_masks.shape[0] > 0:
            ...
            #print("🟡 loss_prj_term (projection loss):", loss_prj_term.item())  # scalar
            pairwise_losses = compute_pairwise_term(
                src_masks, k_size, 2
            )
            #print("🟣 pairwise_losses shape:", pairwise_losses.shape)  # Expected: [N*T, H, W]
        ...
        del src_masks
        del target_masks
        return losses["loss_bound_neighbor"], losses["loss_bound"], losses["loss_mask"]
