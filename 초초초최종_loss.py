...
class MultiStepMultiMasksAndIous(nn.Module):
    ...
    def convert_predict(outs_batch: List[Dict]) -> torch.Tensor:
    """
    Convert SAM2 outputs (multi-step, multi-mask per frame) to [N, T, H, W] format
    suitable for MaskFreeVIS-style processing.
    """
    ...
    # pred_masks_per_frame: List[(N, H, W)] of length T
    # Stack into Tensor: (T, N, H, W)
    ...
    def convert_gt(gt_masks: torch.Tensor) -> torch.Tensor:
    """
    Convert ground-truth masks from [T, N, H, W] → [N, T, H, W]
    Assumes that object indices are aligned across frames.

    Args:
        gt_masks (Tensor): Ground truth masks of shape [T, N, H, W]

    Returns:
        Tensor: Reordered masks of shape [N, T, H, W]
    """
